...
class VcRegistrarPagos extends Component
{
    ...
    public function loadData(){
        ...
        foreach ($personas as $data)
        {
            ...
            foreach ($planilla as $recno){
                ...
                if ($recno['tipo']=='P' & $recno->rubrosrol['imprimerol1']==1) {
                    $this->otringr = $this->otringr + $recno['valor'];
                }

                if ($recno['tipo']=='D' & $recno->rubrosrol['imprimerol1']==1) {
    ...
    public function procesar(){
        ...
        $objperiodo = TmPeriodosrol::find($tcrolPago->periodosrol_id);
        $objperiodo->Update([
            'aprobado' => 1,
        ]);
    ...
    {   
        ...
        /* Total Nomina */
        $objTotal = TdRolPagos::query()
        ->join("tm_rubrosrols as r","r.id","=","td_rol_pagos.rubrosrol_id")
        ->selectRaw('td_rol_pagos.rubrosrol_id, sum(valor) as valor, td_rol_pagos.tipo')
        ->where([
            ['td_rol_pagos.rolpago_id',$this->objWhere['rolpagoId']],
            ['valor','>',0],
            ['r.imprimerol1','=',1],
        ])
        ->groupBy('td_rol_pagos.rubrosrol_id','td_rol_pagos.tipo')
        ->get();
        
        $totNomina = [];
        $valorIng = 0.00;
        $valorEgr = 0.00;
        foreach ($objTotal as $rubro){
            if ($rubro->tipo=='P'){
                $totNomina[0][$rubro->rubrosrol_id] = $rubro['valor'];
                $valorIng = $valorIng+$rubro['valor'];
            }
        }
        $totNomina[0]['TOTING'] = $valorIng;

        foreach ($objTotal as $rubro){
            if ($rubro->tipo=='D'){
                $totNomina[0][$rubro->rubrosrol_id] = $rubro['valor'];
                $valorEgr = $valorEgr+$rubro['valor'];
            }
        }
        $totNomina[0]['TOTEGR'] = $valorEgr;
        $totNomina[0]['TOTPAG'] = $valorIng-$valorEgr;
        /* Total Nomina */
        ...
        return $pdf->setPaper('A4','landscape')->stream('Nomina General.pdf');
    }
    ...
    public function downloadPagosPDF($objdata)
    { 
        $data = json_decode($objdata);
        ...
        $total = array_sum(array_column($tblrecords, 'TOTPAG'));

        $pdf = PDF::loadView('reports/lista_pagos',[
            'tblcia' => $objCia,
            'roldatos' => $objDatos,
            'tblrecords' => $tblrecords,
            'total' => $total,
        ...
        return $pdf->setPaper('A4')->stream('Lista de Pagos.pdf');
